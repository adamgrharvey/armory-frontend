...
export default function characterStringSplitter(
  characterString,
  setCharacter,
  setCharacterExists,
  setLoading,
...
const PvPStringSplitter = function (PvPString) {
  ...
  for (let i = 0; i < PvPArr.length; i++) {
    ...
    PvP[brackets[i]] = {
      rating: Number(hexToDecimal(rating)),
      seasonPlayed: Number(hexToDecimal(seasonPlayed)),
      seasonWon: Number(hexToDecimal(seasonWon)),
    }
  }
  ...
  for (const stat of statisticsArr) {
    ...
    if (statisticID !== '') {
      if (statisticValues && statisticValues.includes('_')) {
        ...
        statistics[hexToDecimal(statisticID)] = {
          stat_id: hexToDecimal(statisticID),
