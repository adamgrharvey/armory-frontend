...
export default function ItemSlot(props) {
  ...
  if (props.item && itemMedia === '') {
    getItemMedia(props.item.id, accessToken, props.setCharLoading, props.slotID).then((res) =>
      setItemMedia(res)
    )
  }
  ...
  return props.item ? (
    <div>
      <div
        onMouseOver={() => {
          props.onMouseEvent(true, props.item)
        }}
        onMouseLeave={() => {
          props.onMouseEvent(false, props.item)
        }}
        className={`ItemSlot ${props.section}`}
      >
        <img
          src={itemMedia}
          alt={props.slotID}
          className={`ItemSlotIcon ${props.item.preview_item.quality.name}`}
        />
        <div
          onMouseOver={() => {
            props.onMouseEvent(true, props.item)
          ...
          className={`ItemSlot ItemDetails ${props.item.preview_item.quality.name}`}
        >
          {props.item.name}
        </div>
      </div>
    ...
    <div className={`ItemSlot ${props.section}`}>
      <img
        src={require(`../images/EmptySlots/inventoryslot_${slotIDs[props.slotID]}.jpg`)}
        alt={props.slotID}
        className="ItemSlotIcon Poor"
      />
