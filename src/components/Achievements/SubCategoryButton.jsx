    ...
    return (
      <div>
        {subCategories[props.category].map((i) => (
          ...
          >
            <button
              className="SubCategoryButtonBG AllCategoryText SubcategoryTextColor"
              onClick={() => {
                props.setSubCategory(i)
              }}
            >
              <div
                className={props.hover === i || props.subCategory === i ? 'Highlight' : ''}
              ></div>
              <div className="Absolute">{i}</div>
            </button>
          </div>
        ))}
      </div>
